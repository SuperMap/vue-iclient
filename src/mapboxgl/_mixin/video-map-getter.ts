...
export default class MapGetter extends Vue {
  ...
  loadMap(targetName) {
    ...
    this.videoMap = mapEvent.$options.getMap(targetName);
    // @ts-ignore
    this.viewModel &&
      typeof this.viewModel.setVideoMap === 'function' &&
      this.viewModel.setVideoMap({ videoMap: this.videoMap, mapTarget: targetName });
    callHook(this, 'loaded', this.videoMap, targetName);
    // 控制与map组件同级的组件的显示加载
    this.$nextTick(() => {
      /**
       * @event loaded
  ...
  removeMap(videoMap = this.videoMap, target = this.getTargetName()) {
    if (videoMap) {
      this.viewModel && this.viewModel.removed && this.viewModel.removed();
      callHook(this, 'removed', videoMap, target);
      this.videoMap = null;
      this.firstDefaultTarget = null;
    }
  }

  loadMapSucceed(videoMap, target) {
    const targetName = this.getTargetName();
