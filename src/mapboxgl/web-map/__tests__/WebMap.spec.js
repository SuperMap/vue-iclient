...
describe('WebMap.vue', () => {
  ...
  xit('initial zxytile layer extent not world ', async done => {
    const data = {
      ...zxytilelayer
    };
    const fetchResource = {
      'https://fakeiportal.supermap.io/iportal/web/config/portal.json': iportal_serviceProxy,
      'https://fakeiportal.supermap.io/iportal/web/maps/test/map.json': data
    };
    mockFetch(fetchResource);
    const spy = jest.spyOn(mapboxgl, 'Map');
    wrapper = mount(SmWebMap, {
    ...
    wrapper.vm.viewModel.on({ addlayerssucceeded: callback });
    const getCRS = jest.spyOn(wrapper.vm.map, 'getCRS');
    getCRS.mockReturnValue(() => {
      console.log('getCRS');
      return {
        getExtent: () => [8182092.307606857, 705312.2300712126, 16613576.019444067, 7087311.00490398]
      };
    });
    await mapWrapperLoaded(wrapper);
