 ...
 * which accompanies this distribution and is available at http://www.apache.org/licenses/LICENSE-2.0.html. */
 ...
 export default class WebMapViewModel extends mapboxgl.Evented {
   ...
   _getServiceInfoFromLayer(layerIndex, layer, dataItemServices, datasetName, featureType, info?: any) {
                   ...
                   this._getDataService(serverId, datasetName).then(data => {
                     ...
                     if (isMapService) {
                       ...
                       this._isMvt(dataService.address, datasetName)
                         ...
                         .catch(() => {
                           //判断失败就走之前逻辑，>数据量用tile
                           this._getServiceInfoFromLayer(index, layer, dataItemServices, datasetName, featureType);
                         });
                     } else {
                       this._getServiceInfoFromLayer(index, layer, dataItemServices, datasetName, featureType);
                     }
   ...
   private _addLayer(layerInfo: any, features: any, index: number | string): void {
     ...
     if (layerInfo.labelStyle && layerInfo.labelStyle.labelField && layerInfo.layerType !== 'DATAFLOW_POINT_TRACK') {
       ...
       // 不在 _addLabelLayer 里面处理 layerAdded++ 的原因：_createDataflowLayer 也调用了 _addLabelLayer ，会导致layerAdded > expectLayerLen
       this._addLayerSucceeded();
     }
   }
