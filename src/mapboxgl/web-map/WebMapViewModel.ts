...
export default class WebMapViewModel extends mapboxgl.Evented {
        ...
        if (mapboxgl.CRS.get(this.baseProjection)) {
          ...
          this.map.on('load', () => {
            ...
            if (!layers || layers.length === 0) {
              ...
              this._addLayers(mapInfo.baseLayer, layers, _taskID);
            }
          });
        } else {
  ...
  private _createWMTSLayer(layerInfo: any): void {
    ...
    this._filterWMTSIsMatched(layerInfo, (isMatched, matchMaxZoom) => {
      ...
      isMatched && this._addBaselayer([wmtsUrl], layerId, 0, matchMaxZoom);
    });
  }
  ...
  _getServiceInfoFromLayer(layer, dataItemServices, datasetName, featureType, info?: any) {
      ...
      if (service && isMapService && service.serviceType === 'RESTMAP') {
        ...
        this._getTileLayerInfo(service.address).then(restMaps => {
          restMaps.forEach(restMapInfo => {
            ...
            // 不在_createBaseLayer里面处理 layerAdded++ 的原因：_addBaseMap 也调用了 _createBaseLayer，会导致layerAdded > expectLayerLen
   ...
   */
  private _addLayers(baseLayer, layers: any, _taskID): void {
    // 存储地图上所有的图层对象
                  ...
                  this._getDataService(serverId, datasetName).then(data => {
                    ...
                    if (isMapService) {
                      ...
                      this._isMvt(dataService.address, datasetName)
                        .then(info => {
                          this._getServiceInfoFromLayer(layer, dataItemServices, datasetName, featureType, info);
                        })
                        .catch(() => {
                          //判断失败就走之前逻辑，>数据量用tile
                          this._getServiceInfoFromLayer(layer, dataItemServices, datasetName, featureType);
                        });
                    } else {
                      this._getServiceInfoFromLayer(layer, dataItemServices, datasetName, featureType);
                    }
   ...
   */
  private _addLayer(layerInfo: any, features?: any): void {
