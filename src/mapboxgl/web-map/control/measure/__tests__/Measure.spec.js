        ...
        measureWrapper.vm.$on("loaded", () => {
            try {
                ...
                measureWrapper.vm.$nextTick(() => {
                    ...
                    mapWrapper.vm.map.fire('mousemove', data),
                        expect(measureWrapper.vm.viewModel.tipHoverDiv.text).toBe("896730.1513 米");
                        ...
                        measureWrapper.vm.$nextTick(() => {
                            ...
                            measureWrapper.vm.viewModel.on("measure-finished", (measureResult) => {
                                ...
                                measureWrapper.vm.$nextTick(() => {
                                    measureWrapper.find('i.ant-select-arrow-icon').trigger('click');
                                    measureWrapper.vm.$nextTick(() => {
                                        measureWrapper.findAll('.ant-select-dropdown-menu li').at(0).trigger('click');
                                        measureWrapper.vm.$nextTick(() => {
                                            expect(measureWrapper.vm.viewModel.tipHoverDiv.text).toBe("203361.79475030015 平方千米");
                                            done()
                                        })
                                    })
                                })
