...
export default class WebMapViewModel extends WidgetViewModel {
    ...
    _getMapInfo(url) {
        ...
        SuperMap.FetchRequest.get(mapUrl, null, { withCredentials: this.withCredentials }).then(response => {
    ...
    _addLayers(layers) {
        ...
        layers.forEach((layer, index) => {
            if ((layer.dataSource && layer.dataSource.serverId) || layer.layerType === "MARKER") {
                ...
                // 获取图层数据
                serverId && SuperMap.FetchRequest.get(url, null, { withCredentials: this.withCredentials }).then(response => {
                    return response.json()
                }).then(data => {
