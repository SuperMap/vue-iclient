...
import type { AreaUnits, Units as LengthUnits } from '@turf/helpers';
import type { Draw } from 'vue-iclient-controllers-mapboxgl/src/types/DrawEvent';
...
export type Units = AreaUnits | LengthUnits
...
class MeasureViewModel extends mapboxgl.Evented {
    ...
    if (this._isDrawing()) {
      ...
      switch (this.activeMode) {
        case 'draw_line_string':
          ...
          this.result = convertLength(tempLength, 'kilometers', this.activeUnit as LengthUnits);
        ...
        case 'draw_polygon':
          ...
          this.result = convertArea(tempArea, 'meters', this.activeUnit as AreaUnits);
  ...
  updateUnit(unit: Units) {
    if (Object.values(this.lenTipNodesList).length) {
        ...
        if (tipNodes && tipNodes.length && ((this.continueDraw && id !== 'id') || id === 'id')) {
          for (let i = 1; i < tipNodes.length; i++) {
            let transValue: number | string = convertLength(
              ...
              this.cacheLengthUnitList[id][i - 1].unit as LengthUnits,
              unit as LengthUnits
            );
      ...
      this.result && (this.result = convertLength(this.result, this.activeUnit as LengthUnits, unit as LengthUnits));
    } else if (this.cachePolygonUnit.value && this.cachePolygonUnit.unit) {
      let transValue = convertArea(+this.cachePolygonUnit.value, this.cachePolygonUnit.unit as AreaUnits, unit as AreaUnits);
      let uniti18n = this.geti18n().t(`unit.square${unit}`);
      this.tipHoverDiv && this.tipHoverDiv.setText(`${transValue} ${uniti18n}`);
      this.result && (this.result = convertArea(this.result, this.activeUnit as AreaUnits, unit as AreaUnits));
    }
  ...
  _renderPopupTip(e: DrawCreateEvent) {
    ...
    if (this.measureNodes.length > 1) {
      ...
      let calcValue = convertLength(tempLength, 'kilometers', this.activeUnit as LengthUnits);
    ...
    popup.setLngLat([lng, lat] as LngLatLike);
