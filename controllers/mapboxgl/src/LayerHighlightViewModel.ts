...
import { transformCoodinates } from 'vue-iclient-core/utils/epsg-define';
...
export default class HighlightLayer extends mapboxgl.Evented {
  ...
  private async getWfsGeojson(url, datasetName, proj, filter) {
    ...
    const featureResponse = await FetchRequest.get(getFeaturesUrl, null, { withoutFormatSuffix: true });
  ...
  private async getWFSCapability(url: string): Promise<any> {
    ...
    const featureResponse = await FetchRequest.get(requestUrl, null, { withoutFormatSuffix: true });
    const featureXml = await featureResponse.text();
