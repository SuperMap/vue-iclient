...
const emit: IndicatorEmits = defineEmits(['click', 'indicatorNumChange'])
...
const fontUnit = computed(() => {
  ...
  const fontUnit = props.fontSize && isNaN(+props.fontSize) ? (props.fontSize as string).replace(reg, '\$2') : 'px';
  return fontUnit;
});
...
const direction = computed(() => {
  return { vertical: 'column', horizontal: 'row' }[props.mode];
});
...
const partsOfPropsWatcher = () => {
  ...
  propsFields.forEach(prop => {
    watch(() => props[prop], function(next) {
      switch (prop) {
        case 'title':
...
const fetchData = ({ features }) => {
  if (features && !!features.length) {
    ...
    unitData.value = Object.prototype.hasOwnProperty.call(properties, props.unitField) ? properties[props.unitField] : props.unit;
    Object.prototype.hasOwnProperty.call(properties, props.numField)
      ? changeNumData(properties[props.numField])
      : changeNumData(props.num);
    titleData.value = Object.prototype.hasOwnProperty.call(properties, props.titleField) ? properties[props.titleField] : props.title;
  }
};
const getData = () => {
...
const changeIndicatorColor = (color) => {
  indicatorColorData.value = color;
}
...
watch(() => props.indicatorColor, (val) => {
...
onBeforeUnmount(() => {
  restService && restService.remove('getdatasucceeded'); 
});
